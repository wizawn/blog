...
class TechnicalIndicators:
    ...
    def __init__(self, client: Optional[BinanceClient] = None):
        ...
        # 初始化K线缓存
        try:
            from ..utils.kline_cache import get_kline_cache
            self.cache = get_kline_cache(ttl=60)
        except:
            self.cache = None
    ...
    def get_klines(self, symbol: str, interval: str, limit: int = 100) -> List[Dict]:
        ...
        try:
            ...
            # 保存到缓存
            if self.cache:
                self.cache.set(symbol, interval, limit, formatted_klines)
    ...
    def calculate_rsi(self, prices: List[float], period: int = 14) -> List[float]:
        ...
        if avg_loss == 0 and avg_gain == 0:
            rsi.append(50)  # 无变化时RSI为50
        elif avg_loss == 0:
            rsi.append(100)  # 只有上涨，RSI为100
        elif avg_gain == 0:
            rsi.append(0)  # 只有下跌，RSI为0
        else:
