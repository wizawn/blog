        ...
        if log_file.exists():
            try:
                with open(log_file, 'r', encoding='utf-8', errors='ignore') as f:
                    ...
                    lines = f.readlines()
                    # 取最后的行数
                    start_index = max(0, len(lines) - limit * 2)  # 多读一些以防格式问题
                    lines = lines[start_index:]

                # 解析日志格式: 2024-03-08 15:00:00 - module - LEVEL - message
                log_pattern = re.compile(r'^(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})[,\s]+[-\s]+([^\s]+)\s+-\s+(\w+)\s+-\s+(.+)$')
