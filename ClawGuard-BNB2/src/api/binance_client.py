...
class BinanceClient:
    ...
    def close(self):
        ...
        if hasattr(self, 'session') and self.session:
            try:
                self.session.close()
                self.logger.debug("BinanceClient Session 已关闭")
            except Exception as e:
                self.logger.error(f"关闭 Session 失败: {e}")
        ...
        try:
            ...
            if e.response is not None:
                try:
                    ...
                    # JSON 解析失败
                    self.logger.error(f"无法解析错误响应: {e}")
                except Exception as parse_error:
                    self.logger.error(f"解析错误响应时出错: {parse_error}")
    ...
    def place_market_order(self, symbol: str, side: str, quantity: float = None,
        ...
        if side == 'BUY':
            # 买入时使用金额
            qty = quote_order_qty if quote_order_qty else quantity
            return self.place_order(symbol, 'BUY', 'MARKET', quote_order_qty=qty)
        else:
            # 卖出时使用数量
            return self.place_order(symbol, 'SELL', 'MARKET', quantity=quantity)
