...
class GridStrategy:
    ...
    def __init__(self,
        ...
        # 参数验证
        if lower_price <= 0:
            raise ValueError(f"下限价格必须大于0，当前值: {lower_price}")
        if upper_price <= 0:
            raise ValueError(f"上限价格必须大于0，当前值: {upper_price}")
        if upper_price <= lower_price:
            raise ValueError(f"上限价格({upper_price})必须大于下限价格({lower_price})")
        if grid_count <= 0:
            raise ValueError(f"网格数量必须大于0，当前值: {grid_count}")
        if grid_count > 1000:
            raise ValueError(f"网格数量过多({grid_count})，建议不超过1000")
        if investment <= 0:
            raise ValueError(f"投资金额必须大于0，当前值: {investment}")

        # 检查网格步长是否过小
        grid_step = (upper_price - lower_price) / grid_count
        if grid_step < upper_price * 0.0001:  # 步长小于价格的0.01%
            raise ValueError(f"网格步长过小({grid_step:.8f})，可能导致交易费用过高")

        # 检查每格金额是否过小
        amount_per_grid = investment / grid_count
        if amount_per_grid < 10:  # 每格少于10 USDT
            raise ValueError(f"每格金额过小({amount_per_grid:.2f} USDT)，建议至少10 USDT")
        ...
        logger.info(f"  网格步长: ${self.grid_step:.2f}")
