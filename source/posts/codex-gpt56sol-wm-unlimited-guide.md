---
title: "GPT-5.6-Sol 隐藏模型 gpt-5.6-sol-wm 调用指南 - 付费账号在订阅额度内多解锁一个 Sol 级模型（2026 年 8 月实测）"
date: 2026-08-15T10:00:00Z
draft: false
weight: 1
categories: ["AI 工具", "技术教程"]
tags: ["ChatGPT", "Codex", "GPT-5.6-Sol", "隐藏模型"]
description: "2026 年 8 月实测，任意 ChatGPT 付费账号（Plus/Pro/Team）通过 Codex 调用隐藏的 gpt-5.6-sol-wm 模型。不走 API 按量计费、不进 API 消费明细，但仍占用订阅的用量窗口——附完整配置教程与常见误解更正。"
...
兄弟们，今天这个发现属于是重量级的——**GPT-5.6-Sol 有一个隐藏的内部路由模型 `gpt-5.6-sol-wm`，任意付费账号都能调用，而且不走 API 按量计费、不出现在 API 消费明细里**。

这不是什么破解或者注入，而是 OpenAI 自己在模型目录中下发但隐藏的一个条目。我在本机的 Codex 缓存中直接抓到了它，并且在多个账号上实测成功。

**核心结论**：
- ✅ 不是公开 API 模型，不产生 API 按量计费（不烧 platform.openai.com 的美元余额）
- ✅ 不出现在 API（platform）消费明细中
- ✅ Plus、Pro、Team 账号均可用
- ❌ Free 账号会被服务器拒绝
- ✅ 调用返回的内容质量和 Sol 完全一致
- ⚠️ **仍然占用 ChatGPT / Codex 订阅本身的用量额度**（5 小时 / 每周用量窗口），并非"无限不消耗"——务必先读下面的更正说明

> **📌 更正说明（务必先读）**：本文早期版本用过"无限""不消耗额度"这类措辞，**并不准确**，现更正如下。
>
> "额度"在这里其实是**两个完全不同**的东西，别混为一谈：
>
> 1. **API 按量计费额度**（platform.openai.com 的美元余额 / API 消费明细）：WM 确实不走这条——因为它 `supported_in_api: false`，压根不是公开 API 模型。**但这一点对所有通过 Codex 订阅调用的模型都成立，并不是 WM 独有的。** Codex 用的是你 ChatGPT 订阅的授权，本来就不按 token 扣 API 的钱。
> 2. **订阅用量额度**（ChatGPT / Codex 的 5 小时 / 每周用量窗口）：**WM 调用同样会占用这个。** 它和普通 Sol 调用一样计入订阅的限流窗口，用多了照样触顶被限速。
>
> 所以准确的说法是：**WM 让你在订阅额度内多一个 Sol 级模型可选，且不产生额外的 API 美元花费——但它不是"无限"，也不是"零消耗"。** 任何"白嫖无限"的理解都是错的。
...
先纠正一个说法：WM **不是**"零消耗"，它只是不走 **API 按量计费**那条通道。原因：

1. **WM 不是公开 API 模型**：`supported_in_api: false` 意味着它不能通过 platform.openai.com 的 API 调用，自然也不会在 API 账单里产生按 token 计费的记录。
2. **Codex 走的是订阅额度**：Codex Desktop/CLI 用的是你 ChatGPT 订阅的授权，而不是 API key 的美元余额。所以**任何** Codex 模型（不止 WM）都不会出现在 API 消费明细里——这不是 WM 的特权。
3. **API 消费明细中不出现**：我连续使用了一周，platform 的 API 账单页面没有 WM 相关记录——这符合上面两点，属于预期，而不是"漏计费"。
4. **但订阅用量窗口照常计入**：使用 WM 和使用普通 Sol 一样，会消耗你订阅的 5 小时 / 每周用量额度。用量打满时，WM 一样会被限速。它省的是"API 的钱"，不是"订阅的量"。

**推测**：WM 可能是 OpenAI 内部测试或特殊用途的路由别名，被包含在了下发给所有客户端的模型目录中。它的价值在于"多一个 Sol 级模型可选"，而不是"绕过任何计量"。
...
目前没有。服务器的权限判断和本地目录是两层独立逻辑，改本地目录无法补出权限。老实说，Plus 一个月也就 $20，能在订阅额度内用上 Sol 级模型已经很值了。
...
**Plus/Pro/Team 任意付费账号 → 修改本地模型目录 → 指定 gpt-5.6-sol-wm → 在订阅额度内多一个 Sol 级模型可用，且不产生额外 API 费用（但仍占用订阅的 5 小时 / 每周用量窗口，非"无限"）。**
